...
async def download_filt(filt, num_chunks=4):
...
def main():
    ...
    # Download recordings
        ...
        start = time.time()
        asyncio.run(download_filt(params))
        end = time.time()
...
# Retrieves metadata and recordings for a given set of metadata
async def download_meta(meta_path, num_chunks=4):
